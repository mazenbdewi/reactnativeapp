...
class EconomyReportsStore {
    ...
    @action async getCategoryPosts(page) {
        ...
        try {
            const { data } = await axios.get(config.url + `/wp-json/wp/v2/posts?categories=26843&per_page=${page}`);
            runInAction(() => {
                this.postData = [];
...
export default new EconomyReportsStore();