...
class InternationalPressStore {
    ...
    @action async getCategoryPosts(page) {
        ...
        try {
            const { data } = await axios.get(config.url + `/wp-json/wp/v2/posts?categories=26716&per_page=${page}`);
            runInAction(() => {
                this.postData = [];
...
export default new InternationalPressStore();